...
# Trust dependent on all variables, divided by country
mod2_ep_bycntry <- dt_new %>%
  group_by(cntry) %>%
  nest() %>%
  mutate(
    model = map(data, ~ lm(trstep ~ age_group + polintr_rev + eduyrs + lrscale + stfdem + stfgov, data = .x)),
    summary = map(model, summary)
  )
summary(mod2_ep_bycntry)

# Checking multicollinearity?
library(car)
model_all <- lm(trstep ~ age_group + polintr_rev + eduyrs + lrscale + stfdem + stfgov, data = dt_new)
vif(model_all)

# Visualization
library(broom)

mod2_coefs <- mod2_ep_bycntry %>%
  mutate(tidy = map(model, tidy)) %>%
  unnest(tidy)

ggplot(mod2_coefs, aes(x = term, y = estimate, color = cntry)) +
  geom_point(position = position_dodge(width = 0.5)) +
  geom_errorbar(aes(ymin = estimate - std.error, ymax = estimate + std.error),
                width = 0.2, position = position_dodge(width = 0.5)) +
  coord_flip() +
  labs(title = "Predictors of Trust in the EU by Country",
...
# Trust dependent on age group, divided by country
mod1_un_bycntry <- dt_new %>%
  group_by(cntry) %>%
  nest() %>%
  mutate(
    model = map(data, ~ lm(trstun ~ age_group, data = .x)),
    summary = map(model, summary)
  )
summary(mod1_un_bycntry)
...
mod2_ep_bycntry <- dt_new %>%
  ...
  nest() %>%
  mutate(
    model = map(data, ~ lm(trstun ~ age_group + polintr_rev + eduyrs + lrscale + stfdem + stfgov, data = .x)),
    summary = map(model, summary)
  )
summary(mod2_un_bycntry)
```
