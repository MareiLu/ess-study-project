...
(Most people can be trusted or you can't be too careful) , **euftf**
(European Union: European unification go further or gone too far) ,
**stfedu** (state of education in country nowadays), **stfgov** (How
satisfied with the national government), **stfhlth** (State of health
services in country nowadays), **trstlgl** (Trust in the legal system),
**imwbcnt** (Immigrants make country worse or better place to live),
**rlgatnd_rev** (How often attend religious services apart from special
occations), **gndr** (Gender), **age_group** (Age group), **year** (Ess
round)

*Nochmal überarbeiten* Our hypothesis: Dependend on country, Gender and
Age and differing by Year...

...the more satisfied one is with the state of their own county
nowadays, the more one trusts in the EP.

...the more open towards other people one is, the more one trusts the
EU.

...people who want the European unification to go even further, also
trust more in the EU.
...
Analysis on average across one country and all years. - Trust in the EU
is very similar. - People in Poland are least trusting towards others
and most trusting in Germany. - People in Poland think, that Unification
should go further, in the other two countries as well, but not as much.
While all are very similarly rather indifferent. - The state of
Education seems to be best in Slovenia, and worst in Germany. - People
in Germany are most satisfied with their national government and least
satisfied in Slovenia. - People in Germany trust their legal system way
more than the other two, the others are very close together. - The
Polish health system seems to be the worst and best in Germany. -
Germans trust their legal system far more than the other two countries,
which seem to be very similarly untrusting. - Polish people are most and
slovenians are the least positive towards immigrants. - Polish people
are by far most religious, and least in Germany.
...
-   **N** = 38,159 (after removing 16,368 rows with missing values)
-   **R² = 0.3612** – The model explains \~36% of the variation in EU
    trust, which is quite good for social science research
-   **Adjusted R²** = 0.361 → Shows good explanatory power with minimal
    overfitting
...
-   **`rlgatnd_rev` (ns)**: Religious attendance is **not
    significantly** related to EU trust.
...
```{r}
model_PL <- lm(trstep ~ ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + year + age_group,
               data = filter(dt_filtered, cntry == "PL"))

summary(model_PL)
```

**Model Overview** - N = 9,088 respondents from Poland - R² = 0.292 →
The model explains 29.2% of the variance in trust in the European
Parliament - Residual Std. Error = 2.158 → Slightly higher error than in
the full model

**Main Findings** - stfedu are not statistically significant - trstlgl,
euftf, clsprty and gender are strongest predictors

-   Trust in EU is strongly shaped by political attitudes (esp. EU
    favorability and legal trust).
-   Satisfaction with government and health system also matter.
-   More religious and younger individuals are less trusting of the EU.
-   Men and older adults tend to trust the EU more.
-   Education satisfaction does not influence EU trust in this model.

### Baseline Linear Model for Slovenia

```{r}
model_SI <- lm(trstep ~ ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + year + age_group, 
               data = filter(dt_filtered, cntry == "SI"))

summary(model_SI)
```

**Main Findings** - imwbcnt not statistically significant - trstlgl,
gndr, clsprty stongest predictors

-   Trust in institutions (especially the legal system, government, and
    education) are the main drivers of EU trust.
-   Favorable attitudes toward the EU and religious attendance also
    contribute positively.
-   Unlike Poland, religious people in Slovenia trust the EU more.
-   Older people and recent survey years are associated with less trust.
-   Gender and interpersonal trust also play a smaller, but significant,
    role.

### Baseline Linear Model for Germany

```{r}
model_DE <- lm(trstep ~ ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + year + age_group,
               data = filter(dt_filtered, cntry == "DE"))

summary(model_DE)
```

**Main Findings** - year not statistically significant - stfgov,
trstlgl, gndr strongest predictors

-   This model explains the most variance (45%) among the three
    countries.
-   Legal and government trust are the strongest predictors of trust in
    the EU.
-   All institutional trust variables (education, health, legal,
    government) and political attitudes are important.
-   Older people and respondents from later years show less trust.
-   Men, religious people, and those with positive views of immigrants
    trust more.

## Interaction with Country for Comparative Analysis

-   This tests whether the effect of a variable differs by country.
-   The model shows if any predictor significantly interacts with
    country.
-   This means the model allows each predictor to have different effects
    in Poland and Slovenia, compared to the reference country Germany
    (baseline).
...
**Model Overview** - R² = 0.396 → The model explains \~39.6% of the
variance in trust in the European Parliament - Adjusted R² = 0.396 →
Very close, indicating good model fit without overfitting - Residual
Std. Error = 1.934 → Good precision

See Country-specific patterns

*Germany (Reference Country)* Main effects apply to Germany because it's
the baseline (when cntryPL and cntrySI = 0):

-   **euftf = 0.180** Strongest positive predictor – favorable EU views
    → more trust
-   **stfgov = 0.297** High satisfaction with government strongly boosts
    EU trust
-   **trstlgl = 0.328** Trust in legal system also highly influential
-   **ppltrst = 0.069** Interpersonal trust matters, but smaller effect
-   **stfhlth = 0.028** Trust in health system matters a bit
-   **stfedu = 0.066** Satisfaction with education has a moderate effect
-   **imwbcnt = 0.054** Positive views on immigration → higher EU trust
-   **gndr = 0.242** Gender effect (likely male = more trust, depending
    on coding)
-   **year = -0.019** Declining trust over survey years
-   **age_group = -0.184** Older respondents trust EU less

*Poland: Differences from Germany*

-   **euftf: 0.1798 - 0.0414 = 0.1384** Still positive, but weaker than
    in Germany

-   **stfgov: 0.2972 - 0.1166 = 0.1806** Less effect of government
    satisfaction

-   **trstlgl: 0.3279 - 0.1139 = 0.214** Legal trust matters less than
    in Germany

-   **stfedu: 0.0657 - 0.0636 = 0.002** No real effect in Poland

-   **age_group: -0.184 + 0.242 = 0.058** Reverses direction: older
    people in Poland trust more

-   Summary – Poland:

    -   Trust in institutions still matters, but less than in Germany
    -   Education satisfaction and religious attendance don’t matter
    -   Age pattern is reversed → older people trust EU more

*Slovenia: Differences from Germany* - **euftf: 0.1798 + 0.0214 =
0.2012** Slightly stronger than in Germany - **stfgov: 0.2972 - 0.0864 =
0.2108** Effect still strong but weaker - **trstlgl: 0.3279 - 0.1054 =
0.2225** Similar to Poland - **ppltrst: 0.0685 - 0.0358 = 0.0327** Less
interpersonal trust impact - **stfhlth: 0.0276 + 0.0276 = 0.0552**
Health satisfaction matters more - **age_group: -0.184 + 0.216 = 0.032**
Age trend also reverses like in Poland

-   Summary – Slovenia:
    -   EU favorability is strongest predictor, even more than in
        Germany
    -   Institutional trust effects weaker than Germany but still
        present
    -   Health system satisfaction is more important than in other
        countries
    -   Older people also trust more

*Summary of Main Points* Key Takeaways - EU favorability (euftf) is the
strongest and most consistent positive predictor across all three
countries. - Institutional trust (gov, legal, health, education) drives
EU trust, but the strength varies by country: - Strongest in Germany -
Weaker in Poland and Slovenia - Interpersonal trust (ppltrst) has a
weaker influence in Slovenia - Age effects reverse in Poland and
Slovenia: older people trust the EU more, unlike in Germany. - Year
effect (declining trust over time) is consistent across countries. - R²
improvement from base model (36%) to interaction model (39.6%) suggests
country-specific effects are meaningful.

## Diagnostics & Model Assumptions

*Multivariate Normality* - analysis assumes that the residuals
(differences between predicted and observed values) are normally
distributed - assumption can be assessed by examining histograms or Q-Q
plots of the residuals, or through statistical tests such as
Kolmogorov-Smirnov test

*No multicollienarity* - correlation matrices, coefficients ideally
below 0.8 - Variance Inflation Factor (VIF): values above 10 indicating
problematic multicollinearity

*Homoscedasticity* - the variance of error terms (residuals) should be
consistent across all levels of the independent variables - a
scatterplot of residuals versus predicted values should not display and
discernible pattern, such as a cone-shaped distribution - adressing it
might involve data transformation or adding a quadratic term to the
model
