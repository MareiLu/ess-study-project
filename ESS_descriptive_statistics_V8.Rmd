...
```{r}
model1 <- lm(trstep ~ ppltrst + euftf + freehms + stfedu + stfgov + stfhlth +
               trstlgl + vote + imwbcnt + health + rlgatnd + gndr + year + cntry,
             data = dt_filtered)
summary(model1)
...
- cntry is included to control for national differences
- high intercept because it is the expected value of trstep when all predictor variables are equal to zero (measurement of year makes this unrealistic)

*Political attitudes*
-euftf (+0.21) – “EU should go further” sentiment increases EU trust significantly (Higher values indicate a stronger desire for unification)

-ppltrst (+0.05) – Higher interpersonal trust increases EU trust slightly

-freehms (−0.13) – Belief that homosexuals are not free reduces trust in the EU slightly (Higher values indicate that a respondent disagrees more strongly)

-stfgov (+0.15) – Satisfaction with the government increases EU trust

-trstlgl (+0.30) – Trust in legal system is the strongest predictor → strongly boosts EU trust

*Institutional satisfaction*
-stfedu (+0.05) and stfhlth (+0.04) – Satisfaction with education and health systems both modestly raise EU trust

*Political engagement*
-vote1 (+0.086) – Voting increases trust slightly (relative to non-voters)

-vote3 (+0.60) – Not eligible to vote has much higher trust — possibly reflects younger or marginalized groups

*Demographics*
-imwbcnt (+0.07) – Positive view of immigrants increases EU trust

-health (−0.04) – Worse health lowers EU trust slightly (Lower values indicate higher satisfaction)

-rlgatnd (n.s.) – Religiosity does not significantly affect EU trust (Lower values indicate that a respondent attends more often)

-gndr (+0.28) – Being female (if coded as 1) is associated with higher EU trust

*Time trend*
-year (−0.0229) – EU trust decreases slightly with each ESS round. About 0.023 points per round

*Country fixed effects (relative to Germany)*
-cntryPL (+0.79) – Poles have ~0.8 higher trust in the EU than Germans

-cntrySI (+0.82) – Slovenians also have higher trust than Germans, by a similar margin

*Model Fit*
-R² = 0.358 – The model explains ~36% of the variance in EU trust --> strong for social science data according to Chat

-Residual SE = 1.99 – The typical prediction error is approx. 2 points on a 0–10 scale, which is moderate

-All variables except rlgatnd are statistically significant
...
```{r}
model_PL <- lm(trstep ~ ppltrst + euftf + freehms + stfedu + stfgov + stfhlth +
               trstlgl + vote + imwbcnt + health + rlgatnd + gndr + year, data = filter(dt_filtered, cntry == "PL"))

summary(model_PL)
```

- health and stfedu are not statistically significant
-trstlgl, vote1 and vote3, euftf and gender are strongest predictors

### Baseline Linear Model for Slovenia
```{r}
model_SI <- lm(trstep ~ ppltrst + euftf + freehms + stfedu + stfgov + stfhlth +
               trstlgl + vote + imwbcnt + health + rlgatnd + gndr + year, data = filter(dt_filtered, cntry == "SI"))

summary(model_SI)
```
-imwbcnt, health not statistically significant
-trstlgl and vote3 stongest predictors

### Baseline Linear Model for Germany

```{r}
model_DE <- lm(trstep ~ ppltrst + euftf + freehms + stfedu + stfgov + stfhlth +
               trstlgl + vote + imwbcnt + health + rlgatnd + gndr + year, data = filter(dt_filtered, cntry == "DE"))

summary(model_DE)
```
-year not statistically significant 
-stfgov, trstlgl, vote3, gndr strongest predictors
...
```{r}
model2 <- lm(trstep ~ (ppltrst + euftf + freehms + stfedu + stfgov + stfhlth +
                       trstlgl + vote + imwbcnt + health + rlgatnd + gndr) * cntry,
             data = dt_filtered)
...
-Legal system trust, EU support, and satisfaction with national institutions drive EU trust across all countries, but to varying degrees

-Voting behavior, religiosity, and gender show country-specific effects:
    -Voters distrust in Germany but trust more in Poland/Slovenia
    -Religious people trust EU more in Poland, less in Slovenia
    -Gender gap flips in Slovenia (women trust less)

## Diagnostics & Model Assumptions
Check for:

Linearity (use component+residual plots)
...
Normality of residuals (plot(model) or broom::augment())
