...
We want a robust, not overfitted model which can still be easily interpreted with its amount of variables.
...
To see if we are correct, we first need to find the lambda penalty term that is used for variable selection. For further understanding, it is important to know that the bigger lambda gets, the more coefficients will be set to zero for the variable selection.
Therefore, we fitted the "best lambda model" with the minimal lambda and MSE, which gives the most precise model. With a lambda of 0.0039, there are still 73 variables in the model, which is not well interpretable and probably overfitted.

In addition, we ran a more robust and economic model by using lambda.1se, which makes it easier to generalize. In this model, lambda is set higher at 0.0280 and also allows the MSE to be higher, which leaves us with 37 variables. This is already a lot better to interpret, but would still exceed the frame of this project.

Therefore, in the last step, we fixed the number of variables and looked at whether lambda is still small enough to be used for interpretation. We decided to stay on a lower bound of 10 to 30 variables and tried several constellations. Ran over which variables are colinear or not useful for interpretation, to delete them before running lasso again.
With this, we came to the conclusion that 11 variables with a lambda of 0.0901 will give us the best model for this project.
...
The variables we will work with are: **cntry** (country), **ppltrst** (Most people can be trusted or you can't be too careful) , **euftf** (European Union: European unification go further or gone too far) , **stfedu** (state of education in country nowadays), **stfgov** (How satisfied with the national government), **stfhlth** (State of health services in country
nowadays), **trstlgl** (Trust in the legal system), **imwbcnt**
(Immigrants make country worse or better place to live), **rlgatnd_rev** (How often attend religious services apart from special occations), **gndr** (Gender), **age_group** (Age group), **year** (Ess round)

We create three categories of variables for our comparative analysis:
...
## Summary Statistics of Variable Averages by Country

```{r}
summary_table <- dt_filtered %>%
  group_by(cntry) %>%
  summarise(across(
    c(trstep, ppltrst, euftf, stfedu, stfgov, stfhlth, trstlgl, imwbcnt, rlgatnd_rev, gndr, agea, eduyrs_winsor, polintr_rev), # here use agea, but later always age_group
...
- table displays average values across years for each country
- average trust in the EU is rather similar
- People in Poland are least trusting towards others and most trusting in Germany.
- People in Poland think, that Unification should go further, in the other two countries as well, but not as much. While all are very similarly rather indifferent.
- The state of Education seems to be best in Slovenia, and worst in Germany.
- People in Germany are most satisfied with their national government and least satisfied in Slovenia.
...
## Summary Statistics of Trust in the EU by Country
...
# Graphical representation to visualize the density
ggplot(dt_filtered, aes(x = cntry, y = trstep, color = cntry)) +
  geom_violin(fill = NA, alpha = 0.4, draw_quantiles = c(0.25, 0.5, 0.75)) +
  ...
  labs(title = "Distribution of Trust in the EU by Country",
       x = "Country", y = "Trust (trstep)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

- Mean trust in EU is rather similar across all three countries and slightly below the median
- The median is 5, the middle value of the scale
- Responses are fairly evenly distributed around the median, with the lowest value (0) occurring more frequently than the highest value (10).
...
dt_filtered %>%
  filter(!is.na(trstep), !is.na(year)) %>%
  group_by(cntry, year) %>%
  summarise(mean_trstep = mean(trstep, na.rm = TRUE), .groups = "drop") %>%
  ggplot(aes(x = year, y = mean_trstep, color = cntry, group = cntry)) +
  geom_line(size = 1) +
  geom_point() +
    scale_color_manual(
    values = c(
  ...
  labs(
    title = "Average Trust in the EU Over Time",
    x = "Year",
...
- mutiple high and low outliers in Slovenia 
- Slovenia's Boxplot is the most narrow one, indicating that the data is not very spread out
...
- highest share of respondents selected 5 as their political attitude in all three countries
...
- high fluctuations between countries
- highest disparities between states between 2010 and 2016
...
- Germany surprisingly has lowest satisfaction throughout almost all years
- very sharp decline in satisfaction in Poland in 2020
...
- strikingly low satisfaction with state of health within country in Poland
...
- values from Polish respondents are the lowest among all three countries in all years
...
- in all three countries, the majority of repsondents voted values above 5, indicating they want the unification to go further
- the highest share of respondents wanting the unification to go further is in Poland
...
- strong disparities up until 2014
- after 2014, unification supports merged among all countries and followed the same upward trend since then, with a decline since 2020
...
- we checked for outliers that might have biased results upwards
...
- there were no high outliers for positive immigration attitude among Polish respondents
- immigration attitude is the highest in Poland and peaked in 2020
- Slovenia has the lowest immigration attitudes but saw a sharp increase since 2018
- would be interesting to see why Polish respondents are so welcoming when they are rather right (are respondents who are rather right  also more welcoming, or is the average so high because of the very high immigration attitude among rather left respondents?) but analyzing this question is out of our scope 
...
- highest trust in people in Germany
...
- very sharp decline in trust in people in Poland in 2020
...
- Poland is by far the country with the highest attendance, even though there is a downward trend that peaked in 2020
...
- clearly highest political interest in Germany among all age groups
- Polish respondents seem to have higher interest than Slovenian respondents in all age groups, except for the group 55-64
...
- interest in politics is the highest for respondents from Germany
- interest in Slovenia and Poland is rathr similar with smaller fluctuations and a sharp incline in Poland in 2020
...
dt_cor <- dt_filtered %>% select(trstep, ppltrst, euftf, stfedu, stfgov, stfhlth, trstlgl, imwbcnt, rlgatnd_rev, gndr, age_group, year, freehms_rev, lrscale, polintr_rev, eduyrs_winsor) 
...
- As the ESS data includes large enough sample sizes for robust OLS estimation, and most variables are ordinal, we do not explicitly check for multivariate normality

*No multicollinearity*
- we tested for multicollinearity with the correlation matrices (coefficients ideally below 0.8)
- no strong correlations between control variables
...
- addressing it might involve data transformation or adding a quadratic term to the model
...
- we are using a large sample size, and our main interest is estimating relationships, not precise inference or prediction
- we report robust standard errors in the final model as they are easy to apply and correct for any distortions in standard errors without having to test for heteroskedasticity explicitly


## Baseline Linear Model with all Variables as Predictors
- we estimate a baseline model for a first overview

```{r}
options(scipen = 999)

# With age groups
model1 <- lm(trstep ~ ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + year + age_group + lrscale + polintr_rev + eduyrs_winsor + cntry,
             data = dt_filtered)
summary(model1)

# With quadratic term of age 
model1a <- lm(trstep ~ ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + year + agea + I(agea^2) + lrscale + polintr_rev + eduyrs_winsor + cntry,
             data = dt_filtered)
summary(model1a)

# MSE
mse(model1, data = dt_filtered)
mse(model1a, data = dt_filtered) #better model fit


```

- including a quadratic term of age would improve the model fit
- however, we use the grouped age variable in the further analysis for greater comparability and interpretability

- from the model output, we can say that being female, higher trust in the legal system, support for further EU unification, higher satisfaction with the national government, being younger and having higher political interest are all associated with higher trust in the EU 
- Trust is higher in Poland and Slovenia than Germany, and has declined slightly over time.


## Country-Specific Group Models 
- they are better for comparison and to see country-specific effects
We include year fixed effects:
- each year might include unique events that influence trust in the EU independent from our predictors
- if year is correlated with one of our predictors, this might introduce bias in our estimates
- year fixed effects help isolate the within-year variation explained by our predictors
- we estimate robust standard errors in all models

```{r}
#Checking if year is treated as continuous variable
str(dt_filtered$year)

#Checking what the baseline is
levels(as.factor(dt_filtered$year))
...
coeftest(modelPL, vcov = vcovHC(modelPL, type = "HC1"))

```

- trstlgl, euftf, political interest and gender are strongest predictors
...
coeftest(modelSI, vcov = vcovHC(modelSI, type = "HC1"))
```

-trstlgl, gndr, political interest stongest predictors
...
summary(modelDE)
coeftest(modelDE, vcov = vcovHC(modelDE, type = "HC1"))
...
-The model tests whether the effect of a variable differs by country and shows if any predictor significantly interacts with country
...
```{r}
# Using anweight now to account for different sample sizes

# keine year fixed effects
model3 <- lm(trstep ~ (ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + year + age_group + lrscale  + polintr_rev + eduyrs_winsor) * cntry,
             data = dt_filtered, weights = anweight)

summary(model3)

#Year fixed effects
model4 <- lm(trstep ~ (ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + as.factor(year) + age_group + lrscale  + polintr_rev + eduyrs_winsor) * cntry,
             data = dt_filtered, weights = anweight)
...
### Effect of EU Support on Trust in the EU by Country
...
interact_plot(model3,
  ...
  scale_color_manual(
    values = c("DE" = "#33658a", "PL" = "#f6ae2d", "SI" = "#f26419"),
    labels = c("DE" = "Germany", "PL" = "Poland", "SI" = "Slovenia")
  ) +
  labs(
    title = "Effect of EU Support on Trust in the EU by Country",
...
- tests how EU support relates to EU trust across countries
- stronger support for unification is associated with higher trust in the EU
- highly relevant, significant results and also intuitive

### Effect of Government Satisfaction on Trust in the EU by Country
...
interact_plot(model3,
              ...
              plot.points = FALSE,
              interval = TRUE) +
  scale_color_manual(values = c("DE" = "#33658a", "PL" = "#f6ae2d", "SI" = "#f26419"),
                     labels = c("DE" = "Germany", "PL" = "Poland", "SI" = "Slovenia")) +
  labs(
    title = "Effect of Government Satisfaction on Trust in the EU by Country",
    x = "Government Satisfaction",
    y = "Predicted EU Trust",
    color = "Country"
  ) +
  theme_minimal(base_size = 13) +
...
- Strong negative interaction in Poland
- Surprising result: trust in national gov lowers EU trust in Poland
- significant in model output

### Effect of Political Placement on Trust in the EU by Country
...
interact_plot(model3,
  ...
  scale_color_manual(values = c("DE" = "#33658a", "PL" = "#f6ae2d", "SI" = "#f26419"),
                     labels = c("DE" = "Germany", "PL" = "Poland", "SI" = "Slovenia")) +
  labs(
    title = "Effect of Political Placement on Trust in the EU by Country",
    x = "Political Placement on Left-Right Scale",
    y = "Predicted EU Trust",
    color = "Country"
  ) +
  theme_minimal(base_size = 13) +
...
- in Germany, right-wing people trust EU less and in Slovenia it is the opposite
- politically important

### Effect of Trust in Legal System on Trust in the EU by Country
...
interact_plot(model3,
  ...
  scale_color_manual(values = c("DE" = "#33658a", "PL" = "#f6ae2d", "SI" = "#f26419"),
                     labels = c("DE" = "Germany", "PL" = "Poland", "SI" = "Slovenia")) +
  labs(
    title = "Effect of Trust in Legal System on Trust in the EU by Country",
    x = "Trust in Legal System",
    y = "Predicted EU Trust",
    color = "Country"
  ) +
  theme_minimal(base_size = 13) +
...
- legal trust boosts EU trust more in Slovenia and Poland
- shows relevance of institutional trust
