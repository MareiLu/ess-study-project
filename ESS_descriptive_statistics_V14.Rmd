...
Year... ...the more politically interested and the more left on the
political scale one is, the more they trust the European Parliament.
...the more open towards other people one is, the more one trusts the
EU. ...the more satisfied one is with the state of their own county
nowadays, the more one trusts in the EP.

The more satisfied one is with their own coutnry's government, the more trust in the EU they have.
...
-   in all three countries, the majority of repsondents voted values
    above 5, indicating they want the unification to go further
...
-   we checked for outliers that might have biased results upwards or downwards
-   reasons??
...
-   immigration attitude peaked in Poland in 2020
...
-   Polish respondents seem to have higher interest than Slovenian
...
dt_filtered %>%
  group_by(age_group, cntry) %>%
  summarise(mean_freehms = mean(freehms_rev, na.rm = TRUE), .groups = "drop") %>%
  ...
  labs(
    title = "Support for Free Lives of Gays and Lesbians",
    x = "Age Group",
    y = "Attitudes towards Gays and Lesbians (1-5 Scale)",
    fill = "Country"
  ) +
  theme_minimal()
```
...
dt_filtered %>%
  ...
  labs(
    title = "Average Attitudes Toward Gays and Lesbians Over Time",
    x = "Year",
    y = "Attitudes towards Gays and Lesbians (1-5 Scale)",
    color = "Country"
  ) +
...
- this graph reveals that there was an upward trend over time among all countries and age groups, indicating that respondents became more welcoming towards gays and lesbians
...
## Adding other variables not included in LASSO 
...
contrasts(as.factor(dt_filtered$year))
...
```{r}
table(filter(dt_filtered, cntry == "PL")$year)

summary(filter(dt_filtered, year == 2004))
#anweight

model_check <- lm(trstep ~ ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + as.factor(year) + age_group, 
                  data = dt_filtered, 
                  weights = anweight)
...
nrow(model.frame(model_check))
nrow(filter(dt_filtered, cntry == "PL"))  # Replace with country code


model <- lm(trstep ~ ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + as.factor(year) + age_group, data = filter(dt_filtered, cntry == "PL"), weights = anweight)

summary(model)

########

model_PL <- lm(trstep ~ ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + as.factor(year) + age_group,
               data = filter(dt_filtered, cntry == "PL"))

summary(model_PL)

# With all variables
modelPLa <- lm(trstep ~ ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + as.factor(year) + age_group + lrscale + polintr_rev + eduyrs_winsor,
               data = subset(dt_filtered, cntry == "PL"))

summary(modelPLa)

mse(model_PL, data = subset(dt_filtered, cntry == "PL"))
mse(modelPLa, data = subset(dt_filtered, cntry == "PL"))
```
...
model_SI <- lm(trstep ~ ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + as.factor(year) + age_group,
               data = filter(dt_filtered, cntry == "SI"))

summary(model_SI)

modelSIa <- lm(trstep ~ ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + as.factor(year) + age_group + lrscale + polintr_rev + eduyrs_winsor,
               data = subset(dt_filtered, cntry == "SI"))

summary(modelSIa)

mse(model_SI, data = subset(dt_filtered, cntry == "SI"))
mse(modelSIa, data = subset(dt_filtered, cntry == "SI"))
```
...
model_DE <- lm(trstep ~ ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + as.factor(year) + age_group, 
               data = filter(dt_filtered, cntry == "DE"))

summary(model_DE)

modelDEa <- lm(trstep ~ ppltrst + euftf + stfedu + stfgov + stfhlth + trstlgl + imwbcnt + rlgatnd_rev + gndr + as.factor(year) + age_group + lrscale + polintr_rev + eduyrs_winsor,
